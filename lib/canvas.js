...
/**
 * Alternative canvas-related object, stateful parameters for use across calls may be stored here.
 * Additional and less common functions may be stored in the $c namespace.
 */
$c = {	// Used in lineToPolar
	lastPoint: {
			x: 0,
			y: 0,
	}
...
function fillStyle(color) {
	if (color == 0 || color == "none") $C.fillStyle = 'rgba(0,0,0,0)'
	else if (color) $C.fillStyle = color
 ...
 * @param {Number} rotation Amount, in radians, to rotate
 */
function rotate_radians(rotation){
	$C.rotate(rotation)
}
...
function strokeStyle(color) {
	if (color == 0 || color == "none") $C.strokeStyle = 'rgba(0,0,0,0)'
	else if (color) $C.strokeStyle = color
...
/**
 * Similar to lineTo but accepts t (theta, the angle in radians)
 * and d, distance. Also returns x,y location of new point with 
 * relation to the last point.
 * @param {Number} t Angle at which to draw line
 * @param {Number} d Distance along angle t to draw line
 */
function lineToPolar(t, d){
	var dx = d*Math.acos(t)
	var dy = d*Math.asin(t)
	$C.lineTo($c.lastPoint.x+dx,$c.lastPoint.y+dy)
	$c.lastPoint.x = $c.lastPoint.x+dx
	$c.lastPoint.y = $c.lastPoint.y+dy
	return {x:dx,y:dy}
}
...
function toPrintDataUrl(width,height) {
	...
	Fred.width = width
	Fred.height = height
	Fred.draw(true) // with a custom size
	var url = $C.toDataURL()
	Fred.width = _width
	Fred.height = _height
	return url
